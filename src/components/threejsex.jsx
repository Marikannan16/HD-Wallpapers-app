...
const Example = () => {
  ...
  const day = currentTime.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
  const time = currentTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });

  return (
    <div className='bg-yellow-400 h-screen py-56'>
      <div className="before:absolute before:bg-sky-500 before:w-3 before:h-12 before:top-24 before:-right-2 before:-z-10 before:rounded-2xl before:shadow-inner before:shadow-gray-50 relative w-60 h-60 bg-sky-500 shadow-inner shadow-gray-50 flex justify-center items-center rounded-3xl mx-auto">
        <div className="w-56 h-56 bg-neutral-900 shadow-inner shadow-gray-50 flex justify-center items-center rounded-3xl">
          <div className="flex flex-col items-center justify-center rounded-2xl bg-neutral-900 shadow-inner shadow-gray-50 w-52 h-52">
            <div className="relative flex flex-col justify-around items-center w-44 h-40 bg-neutral-900 text-gray-50">
              <span className="text-sm text-gray-400">{day}</span>
              <span className="z-10 flex items-center text-6xl text-white [text-shadow:_2px_2px_#ff0000,_1px_2px_#ff0000]">
                          {time.split(':')[0]}
                <span className="text-xl font-bold text-gray-50 ms-3">   </span>
                {time.split(':')[1].slice(0, 2)}
                <span className="text-sm">{time.split(' ')[1]}</span>
              </span>
              <div className="text-gray-50 w-48 flex flex-row justify-evenly mt-2">
                <span className="text-xs font-bold">89</span>
                <div className="flex flex-row items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-red-500 animate-bounce" viewBox="0 0 100 100">
                    <path d="M23,27.6a15.8,15.8,0,0,1,22.4,0L50,32.2l4.6-4.6A15.8,15.8,0,0,1,77,50L50,77,23,50A15.8,15.8,0,0,1,23,27.6Z"></path>
                  </svg>
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-current ml-2" viewBox="0 0 100 100">
                    <path d="M80.2,40.7l-1.1-2-.2-.3.3-.3c2.2-14.7-21.3-25.6-20.7-21S57,38.1,45.4,31.8c-9.3-5.1-12.9,12.1-22.8,33.7C16.2,79.4,20.8,82.3,27,81l.3.4L29,83.3a1.4,1.4,0,0,0,1.8.5l.9-.3a1.6,1.6,0,0,0,1.1-1.9l-.5-2.5a38.2,38.2,0,0,0,4.5-2.7L38.6,78a1.8,1.8,0,0,0,2.4-.1l1.2-1.1a1.9,1.9,0,0,0,.4-1.9l-1-2.5L45.5,69l1.7,1.6a1.8,1.8,0,0,0,2.4-.1l.9-1a1.7,1.7,0,0,0,.4-1.8L50,65c5.6-5,11.9-10.9,17.3-15.8l.4.2,1.9,1.1a1.6,1.6,0,0,0,2.1-.2l.8-.8a1.6,1.6,0,0,0,.3-2.1l-1.3-2.1,3.2-3.1,2.2,1.5a1.8,1.8,0,0,0,2.2-.1l.8-.8A1.7,1.7,0,0,0,80.2,40.7Z"></path>
                  </svg>
                </div>
              </div>
            </div>
            <span className="text-gray-700 text-lg font-light mt-2">Kannan</span>
          </div>
        </div>
      </div>
    </div>
